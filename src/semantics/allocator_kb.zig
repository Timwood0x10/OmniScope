...
pub const AllocatorKB = struct {
    ...
    fn populateBuiltin(kb: *AllocatorKB) AllocatorKBError!void {
        ...
        // Rust GlobalAlloc intrinsics (generated by rustc for #[global_allocator]).
        try kb.addBuiltinPair("__rust_alloc", "__rust_dealloc", "rust");
        try kb.addBuiltinPair("__rust_alloc_zeroed", "__rust_dealloc", "rust");
        try kb.addBuiltinPair("__rust_realloc", "__rust_dealloc", "rust");
        try kb.addBuiltinPair("__rdl_alloc", "__rdl_dealloc", "rust");
        try kb.addBuiltinPair("__rg_alloc", "__rg_dealloc", "rust");
...
test "allocator_kb - rust global allocator" {
    var temp_mem = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = temp_mem.deinit();
    const allocator = temp_mem.allocator();
    ...
    try std.testing.expect(kb.isAllocator("__rust_alloc"));
    try std.testing.expect(kb.isAllocator("__rust_alloc_zeroed"));
    try std.testing.expect(kb.isAllocator("__rust_realloc"));
    try std.testing.expect(kb.isDeallocator("__rust_dealloc"));

    try std.testing.expectEqualStrings("__rust_dealloc", kb.findMatchingFree("__rust_alloc").?);
    try std.testing.expectEqualStrings("__rust_dealloc", kb.findMatchingFree("__rust_alloc_zeroed").?);
    try std.testing.expectEqualStrings("__rust_dealloc", kb.findMatchingFree("__rust_realloc").?);

    try std.testing.expect(kb.isAllocator("__rdl_alloc"));
    try std.testing.expect(kb.isDeallocator("__rdl_dealloc"));
    try std.testing.expect(kb.isAllocator("__rg_alloc"));
    try std.testing.expect(kb.isDeallocator("__rg_dealloc"));

    const stats = kb.getStats();
    try std.testing.expect(stats.pair_count > 15);
    try std.testing.expect(stats.allocator_count > 20);
}
