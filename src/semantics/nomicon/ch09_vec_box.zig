...
/// Uses pre-cached fir.returns[] instead of BB traversal.
fn functionReturnsHeapProvenance(fir: *const FunctionIR, srt: *SemanticTree) bool {
    for (fir.returns) |ret_inst| {
        if (c.LLVMGetNumOperands(ret_inst) == 0) continue;
        const ret_val = c.LLVMGetOperand(ret_inst, 0);
...
fn isHeapAllocCall(inst: c.LLVMValueRef, opcode: c_uint) bool {
    ...
    if (@intFromPtr(name_raw) == 0) return false;
    const callee_name = std.mem.sliceTo(name_raw, 0);
...
fn propagateHeapProvenance(ir_store: *const ModuleIRStore, srt: *SemanticTree) u32 {
    var new_marks: u32 = 0;
                ...
                c.LLVMCall, c.LLVMInvoke => {
                    ...
                    if (c.LLVMIsDeclaration(callee) == 0) {
                        ...
                        const callee_name = if (@intFromPtr(callee_name_ptr) != 0)
                            ...
                            "";
                        const callee_fir = ir_store.findFunction(callee_name);
                        if (callee_fir) |cf| {
                            if (functionReturnsHeapProvenance(cf, srt)) {
...
pub fn detect(
    ...
    // Use pre-cached fir.calls[] — no BB/inst traversal needed
    for (ir_store.function_list) |fir| {
        for (fir.calls) |inst| {
            const call_opcode = c.LLVMGetInstructionOpcode(inst);
            if (!isHeapAllocCall(inst, call_opcode)) continue;

            const called_val = c.LLVMGetCalledValue(inst);
            const callee_name = if (@intFromPtr(called_val) != 0) blk: {
                const name_raw = c.LLVMGetValueName(called_val);
                break :blk if (@intFromPtr(name_raw) != 0) std.mem.sliceTo(name_raw, 0) else null;
            } else null;

            const name = callee_name orelse continue;
            const ref = @intFromPtr(inst);

            if (srt.hasKind(ref, .heap_provenance) == null) {
                try srt.recordResolution(ref, .heap_provenance, 0.95, "Ch9 Vec/Box", name);
            }
        }
    }
    ...
    while (iterations < max_iterations) : (iterations += 1) {
        const new_marks = propagateHeapProvenance(ir_store, srt);
