//! Pipeline Runner — Module pipeline orchestration
//!
//! Extracted from main.zig: registerAllPasses, runModulePipeline,
//! deinitAnalyzeResult, runSingleFileAnalysis, runMultiFileAnalysis,
//! generateVisualization, countFunction, and AnalyzeResult.
    ...
    if (matcher.matches.items.len > 0) {
        ...
        for (matcher.matches.items) |*match| {
            ...
            if (ffi_precision.isDangerousFFIPattern(match)) {
                if (ffi_precision.ffiMatchToIssue(match)) |issue| {
