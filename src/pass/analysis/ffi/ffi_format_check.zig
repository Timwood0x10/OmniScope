//! Format String and Constant Value Detection for FFI Safety Analysis
//!
//! This module provides utilities for detecting compile-time constant values
//! in LLVM IR, primarily used to identify safe format string arguments in
//! printf-like FFI calls.
//!
//! Extracted from ffi_boundary.zig to reduce main file size.
//!
//! Key functions:
//! - isFormatStringConstant: Check if a printf-like call's format arg is constant
//! - isConstantValue: Recursively check if an LLVM value derives from a constant
