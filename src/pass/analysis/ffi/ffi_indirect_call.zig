//! Indirect Call Target Resolution for FFI Boundary Detection
//!
//! This module handles the resolution of indirect function calls (function pointers,
//! vtable calls, JNI calls) in LLVM IR back to their concrete function names.
//!
//! Extracted from ffi_boundary.zig to reduce main file size and improve modularity.
//!
//! Key functions:
//! - resolveIndirectCallTarget: Main entry point for indirect call resolution
//! - extractStructNameFromGEPText: Parse struct type names from GEP instructions
//! - mapStructFieldToFunction: Map (struct, field_index) pairs to known function names
