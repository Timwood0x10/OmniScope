...
test "Integration - comprehensive FFI safety check" {
    ...
    const test_cases = [_][]struct {
        ...
        // Safe cases
        .{ .func_name = "pureGoFunc", .is_cgo = false, .has_unsafe = false, 
          .mem_pattern = .safe, .risk_level = .safe },
        
        // Low risk: cgo but properly managed
        .{ .func_name = "C.process_with_KeepAlive", .is_cgo = true, .has_unsafe = true,
          .mem_pattern = .keepalive_guarded, .risk_level = .low },
        
        // Medium risk: cgo with manual memory
        .{ .func_name = "_cgo_allocate_using_Cmalloc", .is_cgo = true, .has_unsafe = true,
          .mem_pattern = .manual_c_memory, .risk_level = .medium },
        
        // High risk: cgo without proper guards
        .{ .func_name = "__cgocallback_handlePointer_noKeepAlive", .is_cgo = true, .has_unsafe = true,
          .mem_pattern = .missing_keepalive, .risk_level = .high },
    };
    ...
    for (test_cases) |tc| {
        ...
        const has_unsafe_result = std.mem.indexOf(u8, tc.func_name, "unsafe") != null or 
                                  isCgoBoundary(tc.func_name);  // Simplified
