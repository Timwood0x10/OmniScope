...
| **FFI Boundary issue type** | ✅ Working | Generated by FFIBoundaryPass after danger-surface dependency fix |
| **Pure C/C++ library analysis** | ⚠️ Not applicable | OmniScope is an FFI analyzer, not a general C/C++ static analyzer |
| **Dependency chain fix** | ✅ Fixed | DangerSurface → ptr-lifetime dependency reversed, eliminating Phase 0 fallback |
...
**Note**: When pure C/C++ libraries (sqlite3, curl, abseil, etc.) have no FFI boundaries, OmniScope's "false positives" are fundamentally use case mismatch, not tool defects. v0.1.9 plans to add custom allocator recognition to improve this.
...
- Cross-language free mismatch detection (feature incomplete, v0.1.9 target)
