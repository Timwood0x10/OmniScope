...
pub fn bug_borrow_escape() {
    ...
    // s dropped here, but C still has the pointer!
}
...
pub fn bug_double_free() {
    ...
    unsafe {
        c_take_ownership(ptr);
        // BUG: C already freed it, this is double free!
        let _ = Box::from_raw(ptr);
    }
}

fn main() {
    println!("=== Cross-Language Ownership Violation Examples ===");
    println!("1. Rust Box -> C Free (bug)");
    bug_rust_box_c_free();
    
    println!("2. Correct ownership transfer");
    correct_ownership_transfer();
    
    println!("3. Borrow escape (bug)");
    bug_borrow_escape();
    
    println!("4. Double free across FFI (bug)");
    bug_double_free();
}
