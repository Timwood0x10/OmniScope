...
OUTDIR="outputs/full_analysis_v017_final"
...
for ll in $FILES; do
  ...
  # Check if file is actually bitcode (.bc format)
  file_type=$(file -b "$ll")
  bcfile=""
  
  if echo "$file_type" | grep -q "LLVM bitcode"; then
    # File is already in .bc format, just copy it
    bcfile="/tmp/${name}.bc"
    cp "$ll" "$bcfile"
    BC_DIRECT=$((BC_DIRECT+1))
  else
    # Try to convert with LLVM 22
    bcfile="/tmp/${name}.bc"
...
printf "  LLVM 22 converted:     %d\n" $((OK_COUNT + CRASH_COUNT - BC_DIRECT))
printf "  Conversion failures:   %d\n" $CONV_FAIL
printf "  Files analyzed OK:     %d (%.1f%%)\n" $OK_COUNT "$(echo "scale=1; $OK_COUNT * 100 / $TOTAL_FILES" | bc)"
...
ls "$OUTDIR/*.json" 2>/dev/null | wc -l | xargs echo "JSON result files:"
ls "$OUTDIR/*.err.log" 2>/dev/null | wc -l | xargs echo "Error log files:"
