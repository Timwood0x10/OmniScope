...
| **cross_lang_free_mismatch** | ❌ Not working | Detection too narrow (Rust→C only, wrong symbol) |
| **FFI Boundary issue** | ❌ Not working | Issue type never generated by any pass |
...
**v0.2.0 Roadmap**:
- Fix `cross_lang_free_mismatch` detection (currently Rust→C only, depends on wrong symbol)
- Add custom allocator recognition (sqlite3_malloc, curl_easy_cleanup, etc.)
- Add FFI Boundary issue type generation
