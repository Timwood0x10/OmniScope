...
> **审计日期**: 2026-05-08  
> **工具**: OmniScope v0.1.8 (LLVM IR Static Analyzer)  
...
## v0.1.8 更新 — memory_graph 修复影响

`memory_graph` 函数名修复 (`src/pass/analysis/pointer_ownership.zig:64-74`) 消除了一个关键 bug：所有 MemoryGraph 来源的 issue 之前被错误地合并到 `"memory_graph"` 一个函数名下，导致真实检测数量被严重低估。

修复方式：通过 `@ptrFromInt(inst)` → `LLVMGetInstructionParent` → `LLVMGetBasicBlockParent` → `LLVMGetValueName` 链从 u64 指令指针恢复真实函数名。
...
```
  ...
  基准测试 (6 文件): 96/96 TP, 0 FP, 0 FN
```

所有数据来自 `corpus/**/*.bc/.ll` 实际运行，可重复验证。

---

**报告版本**: v2.0 (Evidence Ladder Format + v0.1.8 Audit)  
**方法论**: OmniScope Static Analysis + Manual Source Verification  
**状态**: ✅ S+ 认证 (Precision 100%, Recall 100%)  
**基准测试**: 6 corpus 文件全部通过
